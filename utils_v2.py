...
def get_raw_train_test_data(df,attrs,time_columns,time_key,weather_attrs = ['humidity','pressure','temperature'],length=24*7*3):
    '''
    ...
    for key,group in grouped:
        ...
        for i in range(0,group.shape[0]-length-48):
            ...
            #add weather attrs last 48 * 2 hours
            weather_attrs_in_window = group[weather_attrs].values[i+length-48:i+length+48,:].flatten().tolist()
            tmp += weather_attrs_in_window			
            #add the label 
...
def get_weather_stat(data):
	data = data.astype(np.float)
	stat_all = []
	#daily stat for each weather attribute
	for i in range(3):
		attr_stat = []
		for j in range(0,4):
			attr_day_stat = single_statistical_features(data[:,i*48*2 + j*24:i*48*2 +(j+1)*24])
			attr_stat.append(attr_day_stat)
		stat_all += attr_stat
	return np.hstack(stat_all)
...
def get_all_features(data,attr,length):
    ...
    weather_features_raw = data[:,6+length*3 :6+length*3 + 48 * 2 * 3]
    weather_stat = get_weather_stat(weather_features_raw)
    ...
    return np.hstack([stations_features,date_features,weather_features_raw,weather_stat,holiday_features,raw_features,stat_features]) 
